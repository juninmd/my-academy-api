...
export class Exercise {
  @ApiProperty({ example: 1, description: 'The unique identifier of the exercise' })
  id: number;

  @ApiProperty({ example: 'Bench Press', description: 'Name of the exercise' })
  name: string;

  @ApiProperty({ example: 'https://example.com/image.jpg', description: 'Illustrative image URL' })
  image: string;
  ...
  @ApiProperty({ enum: ExerciseType, example: 'STRENGTH', description: 'Type of exercise' })
  type: ExerciseType;
  ...
  @ApiProperty({ required: false, type: () => [Workout], isArray: true, description: 'Related workouts' })
  workouts?: Workout[];
}
